...
class InputGenerationFromLabelTraj:
    def __init__(self, lp_code_file, lp_format_output_file, gene_mode, week_holiday_mode):
        """
    ...
    def lp_calculation(self):
        ...
        minitools.ifFolderExistThenCreate(OUT_PUT_FOLDER)
        ...
        if gene_mode == 'Re_Calculate':
            ...
            for i in tqdm(range(1)):
                ...
                current_lp_df = current_lp_df[current_lp_df['holiday'] == week_holiday_mode].reset_index()  # filter holidays
                hour_state_list = []
                start_flag = False
                ...
                temp_total_lp_code_df.to_csv(OUT_PUT_FOLDER_INDIVIDUAL + 'true_life_pattern_' +lp_file_list[i].split('\\')[-1].split('_')[0] + '.csv')
                total_lp_code_list.append(temp_total_lp_code_df.values)

            minitools.savePKL(total_lp_code_list, OUT_PUT_FOLDER + str(user_num) + '_input_lp_list.pkl')
            return

        elif gene_mode == 'LOAD_EXISTING':
            total_lp_code_list = minitools.loadPKL(OUT_PUT_FOLDER + str(user_num) + '_input_lp_list.pkl')
...
#=================================Down Sampling to form Input==========================================#
    ...
    minitools.savePKL(down_total_lp_code_list, '2000_input_down_lp_list.pkl')
    np.save('2000_input_down_lp_score.npy',lp_score)
    return

#===============================Record the Location ===========================================#
def lpFile2LocDict_Meshcode(current_lp_df):
...
def lpFile2LocDict_Lonlat(current_lp_df):
    home_place_num = len(current_lp_df['home_label_order'].unique())-1
    work_place_num = len(current_lp_df['work_label_order'].unique())-1
    other_place_num = len(current_lp_df['other_label_order'].unique())-1
    home_places = []
...
def lp_list_with_loc_info(
    ...
    for i in tqdm(range(user_num)):
        ...
        temp_lp_location_dict = lpFile2LocDict_Lonlat(current_lp_df)
    ...
    minitools.savePKL(total_key_point_dict, OUT_PUT_FOLDER_INDIVIDUAL + str(user_num) + '_key_point_lonlat.pkl')
    DataFrame(total_key_point_dict).T.to_csv(OUT_PUT_FOLDER_INDIVIDUAL + str(user_num) + '_key_point_lonlat.csv')
    minitools.savePKL(total_lp_code_list, OUT_PUT_FOLDER + str(user_num) + '_input_lp_loc_list.pkl')
    return