...
class LifePattern():
    ...
    def __init__(self, G,true_seqs, index2node, node2index):
        '''
        ...
        unique, counts = np.unique(start_nodes, return_counts=True)
        pob = counts / np.sum(counts)
        self.TravStartNodes = [unique,pob]
...
#         preds = preds +0.00001
    ...
    def travSeq(self,temperature=[1 for x in range(24)]):
        ...
        cr_node_int = self.TravStartNodes[0][cr_node_int] + '.0'
...
def graphVisulize(save_name,uid,temp_graph,edge_attr_name):
...
#     '''
#     )
    net.show(save_name + '_user_'+str(uid)+'.html')
    return net
