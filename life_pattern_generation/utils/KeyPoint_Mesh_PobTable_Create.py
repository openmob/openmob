...
uids = ['%08d' %uid for uid in demo_df.index.values]
h0_mesh_list = []
for uid in uids:
    mesh = ju.to_meshcode(kps[uid]['H_0'][0],kps[uid]['H_0'][1],3)
    h0_mesh_list.append(mesh)
unique, counts = np.unique(h0_mesh_list, return_counts=True)
demo_df['h0_mesh'] = h0_mesh_list
h0_demo_df = demo_df.groupby('h0_mesh').mean()
h0_demo_df['prior_pob'] = counts/np.sum(counts)
h0_demo_df = h0_demo_df[demo_group+['prior_pob']]
h0_demo_df.to_csv('../KeyPoint_PobTable/Home_Mesh_Demo_Table/ho_mesh_demo.csv')
